...
func compileAntigravityMCP(ctx CompileContext, servers map[string]McpServer) ([]types.TrackedFile, error) {
	...
	// Write to ~/.gemini/config/mcp_config.json for Antigravity desktop-IDE
	// compatibility (serverUrl key). This file is read by the Antigravity IDE
	// but is NOT part of the open-source Gemini CLI schema; kept additive.
	...
	// Also write mcpServers into the scope-appropriate settings.json so that
	// the open-source Gemini CLI discovers MCP servers. Remote entries use
	// httpUrl (Streamable HTTP) per the official schema:
	// https://raw.githubusercontent.com/google-gemini/gemini-cli/main/schemas/settings.schema.json
	geminiDir, err := ResolveToolRoot(types.ToolIdAntigravity, ctx.SetupScope, ctx.toAdapterContext())
	if err != nil {
		return ctx.FileRecords, fmt.Errorf("resolve gemini dir: %w", err)
	}
	if err := files.EnsureDir(geminiDir); err != nil {
		return ctx.FileRecords, fmt.Errorf("ensure gemini dir: %w", err)
	}
	settingsPath := filepath.Join(geminiDir, "settings.json")
	bakPreexisted := files.FileExists(settingsPath + ".bak")
	backup, err := configmerge.MergeJSONFile(settingsPath, toGeminiCliMcp(servers))
	if err != nil {
		return ctx.FileRecords, fmt.Errorf("merge %s: %w", settingsPath, err)
	}
	// Remove only a backup THIS call created. settings.json is generated by the
	// antigravity adapter's own Install earlier in this same run, so a freshly
	// created .bak holds no user content and would leak into compiled output. A
	// pre-existing .bak (a real user's original config saved earlier) is kept.
	if backup != "" && !bakPreexisted {
		_ = os.Remove(backup)
	}
	hash, _ = files.FileHash(settingsPath)
	ctx.FileRecords = append(ctx.FileRecords, types.TrackedFile{
		Path: trackedRecordPath(mcpWorkspaceRoot(ctx), settingsPath), Hash: hash, Source: "compiled:mcp:antigravity:settings", Owner: types.FileOwnerUser,
	})
...
// format. Remote servers use the serverUrl key (Antigravity platform-specific;
// not part of the open-source Gemini CLI schema). Kept for desktop-IDE
// compatibility; see also toGeminiCliMcp for the Gemini CLI-compatible form.
...
// toGeminiCliMcp converts servers to the Gemini CLI settings.json mcpServers
// format per the official schema (settings.schema.json). Remote servers use
// httpUrl (Streamable HTTP transport) since McpServer carries no transport-type
// discriminator; httpUrl is the modern MCP default. Stdio servers use
// command/args/env, matching the MCPServerConfig schema exactly.
func toGeminiCliMcp(servers map[string]McpServer) map[string]any {
	mcpServers := make(map[string]any)
