...
| Manifest-driven compile and seven-target contract | `specs/adrs/006-manifest-driven-compile-and-seven-target-contract.md` | Makes `.ai/lazyai.json` + `.ai/lock.json` the V2 compile contract; historical seven-target freeze; partially superseded by ADR-009 for target enumeration |
| Nine-target contract, Codex stable, Cursor beta | `specs/adrs/009-nine-target-contract-codex-stable-cursor-beta.md` | Nine compile targets; Codex stable; Cursor sole beta adapter (issue #612) |
...
| Positional `.lazyai/` discovery replaces the workspace registry; resolution is a real per-field merge (project > workspace > global > default) | `specs/adrs/008-lazyai-config-directory-redesign.md` | Removes the central `~/.lazyai/workspaces.yaml` + `Active`-pointer model and its 6 registry-native commands in favor of directory-position discovery (up to 2 local layers above the global base) and genuine field-level layering, matching `git`/`.vscode`-style conventions and closing the standard doc's 3 previously-open blocking questions; supersedes `specs/001-internal-sidecar/SPEC.md` |
